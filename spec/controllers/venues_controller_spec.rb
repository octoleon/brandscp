...
describe VenuesController, type: :controller do
  ...
  describe "GET 'list_export'", :search, :inline_jobs do
    ...
    it 'should include the results' do
      ...
      event = create(:approved_event, company: company, campaign: create(:campaign, company: company),
                                      start_date: '01/23/2019', end_date: '01/23/2019',
                                      start_time: '10:00 am', end_time: '12:00 pm',
                                      place: venue.place, event_data: build(:event_data))
      create(:event_expense, amount: 15.98, event: event)
      Sunspot.commit

      expect { xhr :get, 'index', scope: 'user', format: :csv }.to change(ListExport, :count).by(1)
      expect(ListExport.last).to have_rows([
        ['VENUE NAME', 'TD LINX CODE', 'ADDRESS', 'CITY', 'STATE', 'SCORE', 'EVENTS COUNT', 'PROMO HOURS COUNT', 'TOTAL $ SPENT'],
        [venue.place.name, '5155520', '123 My Street', 'New York City', 'NY', '0', '1', '2.0', '$15.98']
      ])
    end
  end
end
