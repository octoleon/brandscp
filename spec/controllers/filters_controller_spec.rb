...
RSpec.describe FiltersController, type: :controller do
  ...
  describe 'GET show' do
    ...
    describe 'Analysis / Campaign Summary' do
      it 'returns all the keys for the campaign_summary scope' do
        get :show, id: 'campaign_summary'
        expect(response).to be_success
        expect(json['filters'].map { |b| b['label'] }).to eq([
          'Brands', 'Areas', 'People', 'Event Status', 'Saved Filters'])
      end

      it 'should return the correct items for the People bucket' do
        campaign = create(:campaign, name: 'CFY12', company: company)
        event = create(:event, campaign: campaign, company: company)

        membership = create(:membership, company_user: user.company_users.first, memberable: event)
        company_user.memberships << membership

        team = create(:team, name: 'Team 1', company: company)
        company_user.teams << team

        get 'show', id: :campaign_summary, format: :json
        expect(response).to be_success

        people_filters = json['filters'].find { |f| f['label'] == 'People' }
        expect(people_filters['items'].count).to eql 2
      end
    end
  end
  ...
  describe 'photos' do
    it 'returns the correct buckets' do
      ...
      expect(json['filters'].map { |b| b['label'] }).to eq([
        'Campaigns', 'Brands', 'Activity Types', 'Areas', 'People', 'Tags', 'Media Types', 'Star Rating', 'Active State', 'Saved Filters'])
    end
  end
  ...
  describe 'Results/Activities' do
    it 'should return the correct filters in the right order' do
      ...
      expect(json['filters'].map { |b| b['label'] }).to eq([
        'Activity Types', 'Brands', 'Campaigns', 'Venues', 'Areas', 'Users',
        'Active State', 'Saved Filters'])
    end
  end
  ...
  describe 'Brand Ambassadors Visits' do
    it 'should return the correct buckets in the right order' do
      ...
      expect(json['filters'].map { |b| b['label'] }).to eq([
        'DIVISIONS','Brands', 'Brand Ambassadors', 'Campaigns', 'Areas', 'Cities', 'Saved Filters'])
    end
