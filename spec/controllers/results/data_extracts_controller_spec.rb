...
RSpec.describe Results::DataExtractsController, type: :controller do
  ...
  describe "GET 'list_export'", :search, :inline_jobs do
    ...
    it 'should return an empty csv with the correct headers' do
      create(:event_expense, event: event, amount: 99.99, category: 'Entertainment')
      create(:event_expense, event: event, amount: 50.45, category: 'Fuel')
      data_extract_expense = create(:data_extract_event_expense)
      data_extract_expense.update_attributes(columns: %w(campaign_name category created_by amount))
      expect { xhr :get, 'show', id: data_extract_expense.id, format: :csv }.to change(ListExport, :count).by(1)
      export = ListExport.last

      expect(export.reload).to have_rows([
        ['Campaign', 'Category', 'Created By', 'Amount'],
        ['Test Campaign FY01', 'Entertainment', 'Test User', '99.99'],
        ['Test Campaign FY01', 'Fuel', 'Test User', '50.45']
      ])
    end
  end
