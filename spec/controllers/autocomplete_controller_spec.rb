...
describe AutocompleteController, type: :controller, search: true do
  ...
  describe 'Media Gallery' do
    it 'returns the correct buckets in the right order' do
      ...
      expect(json.map { |b| b['label'] }).to eq(['Campaigns', 'Brands', 'Activity Types', 'Places', 'People', 'Media Types', 'Active State'])
    end
    ...
    it 'returns the activity types in the Activity Types Bucket' do
      activity_type = create(:activity_type, name: 'Activity Cacique', company_id: company)
      Sunspot.commit

      get 'show', id: 'results_photos', format: :json, q: 'cac'
      expect(response).to be_success

      activity_types_bucket = json.select { |b| b['label'] == 'Activity Types' }.first
      expect(activity_types_bucket['value']).to eq([{ 'label' => 'Activity <i>Cac</i>ique', 'value' => activity_type.id.to_s, 'type' => 'activity_type' }])
    end
    ...
    it 'returns the media types in the Media Types Bucket' do
      get 'show', id: 'results_photos', format: :json, q: 'ima'
      expect(response).to be_success

      media_types_bucket = json.select { |b| b['label'] == 'Media Types' }.first
      expect(media_types_bucket['value']).to eq([{ 'label' => '<i>Ima</i>ge', 'value' => 'Image', 'type' => 'media_type' }])
    end
  end
  ...
  describe 'Brand Ambassadors Visits' do
    it 'should return the correct buckets in the right order' do
      ...
      expect(json.map { |b| b['label'] }).to eq(%w(Campaigns Brands Places People))
    end
    ...
    it 'should return the brands in the Brands Bucket' do
      campaign = create(:campaign, name: 'Cosmos', company_id: company.id)
      brand = create(:brand, name: 'Imperial', company_id: company.id)
      campaign.brands << brand
      Sunspot.commit

      get 'show', id: 'visits', q: 'impe'
      expect(response).to be_success

      brands_bucket = json.select { |b| b['label'] == 'Brands' }.first
      expect(brands_bucket['value']).to eq([{ 'label' => '<i>Impe</i>rial', 'value' => brand.id.to_s, 'type' => 'brand' }])
    end

    it 'should return the brand portfolios in the Brands Bucket' do
      brand_portfolio = create(:brand_portfolio, name: 'Beers Brands', company_id: company.id)
      Sunspot.commit

      get 'show', id: 'visits', q: 'bee'
      expect(response).to be_success

      brand_portfolios_bucket = json.select { |b| b['label'] == 'Brands' }.first
      expect(brand_portfolios_bucket['value']).to eq([{ 'label' => '<i>Bee</i>rs Brands', 'value' => brand_portfolio.id.to_s, 'type' => 'brand_portfolio' }])
    end
  end
end
