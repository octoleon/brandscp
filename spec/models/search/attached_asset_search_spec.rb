...
describe AttachedAsset, type: :model, search: true do
  ...
  it 'should search for roles' do
    ...
    activity_type1 = create(:activity_type, name: 'POS Drop', company: company, campaigns: [event.campaign])
    activity_type2 = create(:activity_type, name: 'Menu Placement', company: company, campaigns: [event.campaign])
    activity1 = create(:activity, activity_type: activity_type1, activitable: venue,
                       campaign: campaign, company_user: company_user1)
    activity2 = create(:activity, activity_type: activity_type2, activitable: venue2,
                       campaign: campaign, company_user: company_user1)
    event.activities << [activity1, activity2]
    Sunspot.index! [asset, asset2]
    ...
    expect(search(company_id: company.id, activity_type: []))
    .to match_array([asset])
    expect(search(company_id: company.id, activity_type: [activity_type1.id, activity_type2.id]))
    .to match_array([asset])
    expect(search(company_id: company.id, activity_type: [activity_type2.id]))
    .to match_array([asset])
