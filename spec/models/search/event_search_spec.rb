...
describe Event, type: :model, search: true do
  ...
  it 'search retricted by users permissions' do
    ...
    create(:event, campaign: campaign2, place: place2, start_date: '03/22/2013', end_date: '03/22/2013')
  ...
  it 'correctly search on the localized date fields', search: false, sunspot_matcher: true do
    ...
    expect(Sunspot.session).to have_search_params(:with) {
      all_of do
        with(:start_at).less_than_or_equal_to(d.end_of_day)
        with(:end_at).greater_than_or_equal_to(d.beginning_of_day)
      end
    }
    ...
    expect(Sunspot.session).to have_search_params(:with) {
      all_of do
        with(:local_start_at).less_than_or_equal_to(d.end_of_day)
        with(:local_end_at).greater_than_or_equal_to(d.beginning_of_day)
      end
    }
  end
  ...
  describe 'area customizations searches' do
    it 'should return events inside cities included to areas in campaigns' do
      ...
      create(:areas_campaign, area: area_la, campaign: campaign, inclusions: [city_la.id])
