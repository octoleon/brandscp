...
describe Ability, type: :model do
  describe 'abilities' do
    ...
    context 'when it is a super user' do
      ...
      it { is_expected.to be_able_to(:create, Phase) }
      it { is_expected.to be_able_to(:update, Phase) }
      it { is_expected.to be_able_to(:destroy, Phase) }
      it { is_expected.to be_able_to(:index, Phase) }

      it { is_expected.to be_able_to(:create, PhaseCondition) }
      it { is_expected.to be_able_to(:update, PhaseCondition) }
      it { is_expected.to be_able_to(:destroy, PhaseCondition) }
      it { is_expected.to be_able_to(:index, PhaseCondition) }

      it { is_expected.to be_able_to(:create, PhaseActivity) }
      it { is_expected.to be_able_to(:update, PhaseActivity) }
      it { is_expected.to be_able_to(:destroy, PhaseActivity) }
      it { is_expected.to be_able_to(:index, PhaseActivity) }

      it { is_expected.to be_able_to(:create, PhaseActivityCondition) }
      it { is_expected.to be_able_to(:update, PhaseActivityCondition) }
      it { is_expected.to be_able_to(:destroy, PhaseActivityCondition) }
      it { is_expected.to be_able_to(:index, PhaseActivityCondition) }
    end
    ...
    context 'when it is NOT a super user' do
      ...
      describe 'Campaign permissions' do
        it 'can activate/deactivate kpis if has the :activate_kpis permission' do
        ...
        it 'can activate/deactivate activity types if has the :activate_kpis permission' do
          campaign = create(:campaign, company: company)
        ...
        it 'can access campaign and activity builders if has the :read and :view_event_form permissions respectively' do
          campaign = create(:campaign, company: company)
          campaign_in_other_comapany = create(:campaign, company: create(:company))

          expect(ability).not_to be_able_to(:view_campaign_builder, campaign)
          expect(ability).not_to be_able_to(:view_activity_builder, campaign)

          expect(ability).not_to be_able_to(:view_campaign_builder, campaign_in_other_comapany)
          expect(ability).not_to be_able_to(:view_activity_builder, campaign_in_other_comapany)

          user.role.permission_for(:read, Campaign, mode: 'campaigns').save
          user.role.permission_for(:view_event_form, Campaign, mode: 'campaigns').save

          expect(ability).to be_able_to(:view_campaign_builder, campaign)
          expect(ability).to be_able_to(:view_activity_builder, campaign)
      ...
      #  __________  ___ ___    _____    ____________________ _________
      #  \______   \/   |   \  /  _  \  /   _____/\_   _____//   _____/
      #   |     ___/    ~    \/  /_\  \ \_____  \  |    __)_ \_____  \
      #   |    |   \    Y    /    |    \/        \ |        \/        \
      #   |____|    \___|_  /\____|__  /_______  //_______  /_______  /
      #                   \/         \/        \/         \/        \/
      describe 'Phase permissions' do
        it 'can create phases if has the :create permission of campaign' do
          campaign = create(:campaign, company: company)
          phase = create(:phase, campaign: campaign)

          expect(ability).not_to be_able_to(:create, phase)

          user.role.permission_for(:create, Campaign, mode: 'campaigns').save

          expect(ability).to be_able_to(:create, phase)
        end

        it 'can update phases if has the :update permission of campaign' do
          campaign = create(:campaign, company: company)
          phase = create(:phase, campaign: campaign)

          expect(ability).not_to be_able_to(:update, phase)

          user.role.permission_for(:update, Campaign, mode: 'campaigns').save

          expect(ability).to be_able_to(:update, phase)
        end

        it 'can destroy phases if has the :deactivate permission of campaign' do
          campaign = create(:campaign, company: company)
          phase = create(:phase, campaign: campaign)

          expect(ability).not_to be_able_to(:destroy, phase)

          user.role.permission_for(:deactivate, Campaign, mode: 'campaigns').save

          expect(ability).to be_able_to(:destroy, phase)
        end

        it 'can read phases if has the :read permission of campaign' do
          campaign = create(:campaign, company: company)
          phase = create(:phase, campaign: campaign)

          expect(ability).not_to be_able_to(:index, phase)

          user.role.permission_for(:read, Campaign, mode: 'campaigns').save

          expect(ability).to be_able_to(:index, phase)
        end
      end

      #  __________  ___ ___    _____    ____________________     _________  ________    _______  ________  .______________.___________    _______    _________
      #  \______   \/   |   \  /  _  \  /   _____/\_   _____/     \_   ___ \ \_____  \   \      \ \______ \ |   \__    ___/|   \_____  \   \      \  /   _____/
      #   |     ___/    ~    \/  /_\  \ \_____  \  |    __)_      /    \  \/  /   |   \  /   |   \ |    |  \|   | |    |   |   |/   |   \  /   |   \ \_____  \
      #   |    |   \    Y    /    |    \/        \ |        \     \     \____/    |    \/    |    \|    `   \   | |    |   |   /    |    \/    |    \/        \
      #   |____|    \___|_  /\____|__  /_______  //_______  /      \______  /\_______  /\____|__  /_______  /___| |____|   |___\_______  /\____|__  /_______  /
      #                   \/         \/        \/         \/              \/         \/         \/        \/                           \/         \/        \/
      describe 'PhaseCondition permissions' do
        it 'can create phase conditions if has the :create permission of campaign' do
          phase_condition = create(:phase_condition, condition: 'one_before', operator: 'is_not')

          expect(ability).not_to be_able_to(:create, phase_condition)

          user.role.permission_for(:create, Campaign, mode: 'campaigns').save

          expect(ability).to be_able_to(:create, phase_condition)
        end

        it 'can update phase conditions if has the :update permission of campaign' do
          phase_condition = create(:phase_condition, condition: 'one_before', operator: 'is_not')

          expect(ability).not_to be_able_to(:update, phase_condition)

          user.role.permission_for(:update, Campaign, mode: 'campaigns').save

          expect(ability).to be_able_to(:update, phase_condition)
        end

        it 'can destroy phase conditions if has the :deactivate permission of campaign' do
          phase_condition = create(:phase_condition, condition: 'one_before', operator: 'is_not')

          expect(ability).not_to be_able_to(:destroy, phase_condition)

          user.role.permission_for(:deactivate, Campaign, mode: 'campaigns').save

          expect(ability).to be_able_to(:destroy, phase_condition)
        end

        it 'can read phase conditions if has the :read permission of campaign' do
          phase_condition = create(:phase_condition, condition: 'one_before', operator: 'is_not')

          expect(ability).not_to be_able_to(:index, phase_condition)

          user.role.permission_for(:read, Campaign, mode: 'campaigns').save

          expect(ability).to be_able_to(:index, phase_condition)
        end
      end

      #  __________  ___ ___    _____    ____________________        _____  ____________________._______   ____.______________.______________ _________
      #  \______   \/   |   \  /  _  \  /   _____/\_   _____/       /  _  \ \_   ___ \__    ___/|   \   \ /   /|   \__    ___/|   \_   _____//   _____/
      #   |     ___/    ~    \/  /_\  \ \_____  \  |    __)_       /  /_\  \/    \  \/ |    |   |   |\   Y   / |   | |    |   |   ||    __)_ \_____  \
      #   |    |   \    Y    /    |    \/        \ |        \     /    |    \     \____|    |   |   | \     /  |   | |    |   |   ||        \/        \
      #   |____|    \___|_  /\____|__  /_______  //_______  /     \____|__  /\______  /|____|   |___|  \___/   |___| |____|   |___/_______  /_______  /
      #                   \/         \/        \/         \/              \/        \/                                                    \/        \/
      describe 'Phase Activity permissions' do
        it 'can create phase activities if has the :create permission of activity' do
          phase_activity = create(:phase_activity)

          expect(ability).not_to be_able_to(:create, phase_activity)

          user.role.permission_for(:create, Activity, mode: 'activities').save

          expect(ability).to be_able_to(:create, phase_activity)
        end

        it 'can update phase activities if has the :update permission of activity' do
          phase_activity = create(:phase_activity)

          expect(ability).not_to be_able_to(:update, phase_activity)

          user.role.permission_for(:update, Activity, mode: 'activities').save

          expect(ability).to be_able_to(:update, phase_activity)
        end

        it 'can destroy phase activities if has the :deactivate permission of activity' do
          phase_activity = create(:phase_activity)

          expect(ability).not_to be_able_to(:destroy, phase_activity)

          user.role.permission_for(:deactivate, Activity, mode: 'activities').save

          expect(ability).to be_able_to(:destroy, phase_activity)
        end

        it 'can read phase activities if has the :show permission of activity' do
          phase_activity = create(:phase_activity)

          expect(ability).not_to be_able_to(:index, phase_activity)

          user.role.permission_for(:show, Activity, mode: 'activities').save

          expect(ability).to be_able_to(:index, phase_activity)
        end
      end

      #  __________  ___ ___    _____    ____________________      _____  ____________________._______   ____.___________________.___.   _________  ________    _______  ________  .______________.___________    _______    _________
      #  \______   \/   |   \  /  _  \  /   _____/\_   _____/     /  _  \ \_   ___ \__    ___/|   \   \ /   /|   \__    ___/\__  |   |   \_   ___ \ \_____  \   \      \ \______ \ |   \__    ___/|   \_____  \   \      \  /   _____/
      #   |     ___/    ~    \/  /_\  \ \_____  \  |    __)_     /  /_\  \/    \  \/ |    |   |   |\   Y   / |   | |    |    /   |   |   /    \  \/  /   |   \  /   |   \ |    |  \|   | |    |   |   |/   |   \  /   |   \ \_____  \
      #   |    |   \    Y    /    |    \/        \ |        \   /    |    \     \____|    |   |   | \     /  |   | |    |    \____   |   \     \____/    |    \/    |    \|    `   \   | |    |   |   /    |    \/    |    \/        \
      #   |____|    \___|_  /\____|__  /_______  //_______  /   \____|__  /\______  /|____|   |___|  \___/   |___| |____|    / ______|    \______  /\_______  /\____|__  /_______  /___| |____|   |___\_______  /\____|__  /_______  /
      #                   \/         \/        \/         \/            \/        \/                                         \/                  \/         \/         \/        \/                           \/         \/        \/
      describe 'Phase Activity Condition permissions' do
        it 'can create phase activity conditions if has the :create permission of activity' do
          phase_activity_condition = create(:phase_activity_condition, condition: 'locked', operator: 'is')

          expect(ability).not_to be_able_to(:create, phase_activity_condition)

          user.role.permission_for(:create, Activity, mode: 'activities').save

          expect(ability).to be_able_to(:create, phase_activity_condition)
        end

        it 'can update phase activity conditions if has the :update permission of activity' do
          phase_activity_condition = create(:phase_activity_condition, condition: 'locked', operator: 'is')

          expect(ability).not_to be_able_to(:update, phase_activity_condition)

          user.role.permission_for(:update, Activity, mode: 'activities').save

          expect(ability).to be_able_to(:update, phase_activity_condition)
        end

        it 'can destroy phase activity conditions if has the :deactivate permission of activity' do
          phase_activity_condition = create(:phase_activity_condition, condition: 'locked', operator: 'is')

          expect(ability).not_to be_able_to(:destroy, phase_activity_condition)

          user.role.permission_for(:deactivate, Activity, mode: 'activities').save

          expect(ability).to be_able_to(:destroy, phase_activity_condition)
        end

        it 'can read phase activity conditions if has the :show permission of activity' do
          phase_activity_condition = create(:phase_activity_condition, condition: 'locked', operator: 'is')

          expect(ability).not_to be_able_to(:index, phase_activity_condition)

          user.role.permission_for(:show, Activity, mode: 'activities').save

          expect(ability).to be_able_to(:index, phase_activity_condition)
        end
      end
    end
  end
