...
RSpec.describe DataExtract::EventExpense, type: :model do
  describe '#available_columns' do
    ...
    it 'returns the correct columns' do
      expect(subject.exportable_columns).to eql([
        ...
        ['event_status', 'Event Status'], %w(area_name Area), ['place_td_linx_code', 'TD Linx Code'],
        ['place_street', 'Venue Street'], ['place_city', 'Venue City'], ['place_name', 'Venue Name'],
  ...
  describe '#rows' do
    ...
    describe 'with data' do
      before do
        create(:event_expense, amount: 159.15, category: 'Entertainment', event: event, created_at: Time.zone.local(2013, 8, 22, 11, 59))
      end

      it 'returns all the comments in the company with all the columns' do
        event.update_column :areas_ids, [area.id]
        subject.columns = %w(category created_by created_at campaign_name area_name end_date end_time
                             start_date start_time event_status place_street place_city place_name place_state place_zipcode)
        expect(subject.rows).to eql [
          ['Entertainment', nil, '08/22/2013', campaign.name, area.name, '01/01/2014', '08:00 PM',
           '01/01/2014', '06:00 PM', event.event_status, place.street, place.city, place.name, place.state, place.zipcode]
        ]
      end
