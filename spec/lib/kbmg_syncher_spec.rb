    ...
    describe 'campaign with a KBMG API Key' do
      ...
      describe 'with valid key' do
        ...
        before do
          ...
          stub_api_data :event_registrations, 'EVNT1' do
            { 'Success' => true, 'Total' => 2, 'Data' => { 'Registrations' => [
              { 'RegistrationId' => 'REG1', 'PersonId' => 'PER1', 'Attended' => true },
              { 'RegistrationId' => 'REG2', 'PersonId' => 'PER2', 'Attended' => false }
            ] } }
          end

          stub_api_data :event_registrations, 'EVNT2' do
            { 'Success' => true, 'Total' => 2, 'Data' => { 'Registrations' => [
              { 'RegistrationId' => 'REG3', 'PersonId' => 'PER1', 'Attended' => true, 'Rsvp' => true },
              { 'RegistrationId' => 'REG4', 'PersonId' => 'PER2', 'Attended' => false, 'Rsvp' => false }
            ] } }
          end
        ...
        it 'synchs the data for the events' do
          event1 = create(:event, campaign: campaign, start_date: '01/01/2015',
                                  place: place1, end_date: '01/01/2015')
          create(:event, campaign: campaign, start_date: '01/02/2015',
                         place: place2, end_date: '01/02/2015')
          create :place, name: 'Melany\'s Place', state: 'California',
                         zipcode: '12233', street_number: '1233', route: 'Union Street'
          ...
          expect(event1.invites.map(&:venue_id)).to include(Venue.last.id)
          ...
          # Run it twice to make sure it doesn't duplicate the data
          expect(subject).to receive(:valid_campaign_api_key?).and_return(true)
          expect do
            subject.process
        ...
        it "matches the account using the persons's zip code" do
          ...
          place1 = create :place, name: '12th & Broad', zipcode: '11122'
          place2 = create :place, name: "Melany\'s Place", zipcode: '33456'
          ...
          expect(Invite.joins(:venue).pluck('distinct venues.place_id')).to match_array(
            [place1.id, place2.id])
        end
