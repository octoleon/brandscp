...
feature 'Events section', js: true do
  ...
  feature 'create' do
    ...
    scenario 'user can not go to split  while image is uploading', :inline_jobs do
      visit event_path(event)
      click_js_button 'Record Expense'
      ...
      within visible_modal do
        ...
        expect(page).to have_content('File attached')
        click_js_button 'Create'
        wait_for_ajax(150)
      end
      ensure_modal_was_closed
  ...
  feature 'edit' do
    ...
    let(:permissions) { [[:show, 'Event'], [:index_expenses, 'Event'],
                         [:create_expense, 'Event'], [:edit_expense, 'Event']] }
    let(:rejected_event) { create(:rejected_event, start_date: Date.yesterday.to_s(:slashes),
                                  end_date: Date.today.to_s(:slashes),
                                  campaign: campaign, place: place,company: company, active: true) }
    let(:event_expense) { create(:event_expense, amount: 500, event: rejected_event, category: 'Phone') }
    let(:brand1) { create(:brand, name: 'Brand 1', company_id: company.id) }
    let(:brand2) { create(:brand, name: 'Brand 2', company_id: company.id) }

    before do
      campaign.brands << [brand1, brand2]
      rejected_event.campaign.update_attribute(:modules, 'expenses' => {
                                                           'settings' => { 'categories' => %w(Phone) } })
    end

    scenario 'should allow to edit receipt image after PER rejection' do
      event_expense
      visit event_path(rejected_event)
      ...
      ensure_modal_was_closed
      asset = AttachedAsset.last
      expect(asset.file_file_name).to eql 'photo.jpg'
