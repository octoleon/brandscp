...
feature 'Attendance', js: true, search: true do
  ...
  let(:place1) do
    create(:place1, name: 'Sandbar', country: 'US', city: 'Parker',
           state: 'Arizona', is_custom_place: false, reference: nil)
  end
  ...
  let(:event1) do
    create(:late_event, start_date: '11/01/2015', start_time: '04:30 pm',
           campaign: campaign, place: place1)
  end
  ...
  shared_examples_for 'a user that can create invites' do
    ...
    scenario 'can create venue invites several times in a row' do
      visit event_path(event)
      create_invite account: 'Guillermitos Bar', invites: 12
      ...
      within visible_modal do
        ...
        fill_in 'Invites', with: 3
        select_from_autocomplete 'Search for a venue...', 'Guillermitos Bar'

        click_js_button "Send 3 Invitations"
      end
      ensure_modal_was_closed
