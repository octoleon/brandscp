...
feature 'Campaigns', js: true do
  ...
  feature 'Details page', js: true do
    ...
    scenario 'should be able to assign areas and places to the campaign' do
      ...
      within tab do
        # Ensure the area now appears on the list of places
        expect(page).to have_content('San Francisco Area')

        # Test the area removal
        click_js_link 'Remove Area'
        expect(page).to have_no_content('San Francisco Area')
        expect(page).to have_content 'No Places have been added to this Campaign.'
      end

      # Adding a place to the campaign
      ...
      within tab do
        # Ensure the place now appears on the list of places
        expect(page).to have_content('Walt Disney World Dolphin')

        # Test the place removal
        click_js_link 'Remove Place'
        expect(page).to have_no_content('Walt Disney World Dolphin')
        expect(page).to have_content 'No Places have been added to this Campaign.'
      end
    end

    scenario 'should be able to assign areas to the campaign and sustain KPI goals preset' do
      Kpi.create_global_kpis
      area = create(:area, name: 'San Francisco Area', company: company)
      area2 = create(:area, name: 'Los Angeles Area', company: company)
      kpis = campaign.active_kpis
      ...
      within tab do
        find_all('.kpi-goal-field').each do |ff|
          ff.set('3')
        end
      ...
      # Ensure the area now appears on the list of areas
      expect(page).to have_content('Walt Disney World Dolphin')
      #Ensure goals still exist
      expect(page).to have_selector("input[value='3.0']")
    end
