...
feature 'Post Event Data' do
  ...
  shared_examples_for 'a user with permissions to fill post event data' do
    ...
    scenario 'should display correct subtraction value when user edits post event data at first time' do
      create(:form_field_calculation,
             name: 'Calculation Subtract', operation: '-',
             calculation_label: 'SUBTRACT TOTAL',
             options: [
    ...
    scenario 'a user can not put zero in denominator' do
      error_msg = 'You must divide by a number different than 0'
      ...
      fill_in('Divide Opt1', with: 10)
      fill_in('Divide Opt2', with: 0)

      within '.event_results_value' do
        numerator = find_field('Divide Opt1')
        denominator = find_field('Divide Opt2')

        expect(numerator['class']).to include('valid')
        expect(denominator['class']).to_not include('valid')
        expect(find('.calculation-total-amount')).to have_content('Infinity')
        expect(page).to have_content(error_msg)
      end

      fill_in('Divide Opt1', with: 0)
      fill_in('Divide Opt2', with: 10)
      ...
      within '.event_results_value' do
        ...
        expect(numerator['class']).to include('valid')
        expect(denominator['class']).to include('valid')
        expect(find('.calculation-total-amount')).to have_content('0')
        expect(page).to_not have_content(error_msg)
      end
    end
