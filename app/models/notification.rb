  ...
  def self.new_event(user, event, team = nil)
    ...
    if user.notifications.where(path: path).count == 0
      ...
      if user.allow_notification?('new_event_team_email')
        presenter = ApplicationController.helpers.present(event)
        email_message = I18n.translate('notifications_email.new_event', 
          url: Rails.application.routes.url_helpers.event_url(event),
          campaign_name: event.campaign.name,
          date: ActionView::Base.full_sanitizer.sanitize(presenter.date_range_basic_info, :tags => []),
          address: ApplicationController.helpers.place_address(event.place, false, '<br />', '<br />', true),
