class Ability
  ...
  def initialize(user)
    ...
    alias_action :add_kpi, :remove_kpi, :add_activity_type, :remove_activity_type, to: :activate_kpis
    alias_action :reject, to: :approve
    alias_action :post_event_form, :update_post_event_form, :view_activity_builder, to: :view_event_form
    ...
    # Super Admin Users
      ...
      can :manage, [Phase, PhaseActivity, PhaseCondition, PhaseActivityCondition]
    ...
    # A logged in user
      ...
      can :view_campaign_builder, Campaign do |campaign|
        can?(:show, campaign)
      end
      ...
      # Phases permissions
      can :create, Phase do |_phase|
        role.has_permission?(:create, Campaign)
      end

      can :update, Phase do |_phase|
       role.has_permission?(:update, Campaign)
      end

      can :destroy, Phase do |_phase|
        role.has_permission?(:deactivate, Campaign)
      end

      can :index, Phase do |_phase|
        role.has_permission?(:read, Campaign)
      end

      # Phase Activities permissions
      can :create, PhaseActivity do |_phase_activity|
        role.has_permission?(:create, Activity)
      end

      can :update, PhaseActivity do |_phase_activity|
        role.has_permission?(:update, Activity)
      end

      can :destroy, PhaseActivity do |_phase_activity|
        role.has_permission?(:deactivate, Activity)
      end

      can :index, PhaseActivity do |_phase_activity|
        role.has_permission?(:show, Activity)
      end

      # Phase Conditions permissions
      can :create, PhaseCondition do |_phase_condition|
        role.has_permission?(:create, Campaign)
      end

      can :update, PhaseCondition do |_phase_condition|
        role.has_permission?(:update, Campaign)
      end

      can :destroy, PhaseCondition do |_phase_condition|
        role.has_permission?(:deactivate, Campaign)
      end

      can :index, PhaseCondition do |_phase_condition|
        role.has_permission?(:read, Campaign)
      end

      # Phase Activity Conditions permissions
      can :create, PhaseActivityCondition do |_phase_activity_condition|
        role.has_permission?(:create, Activity)
      end

      can :update, PhaseActivityCondition do |_phase_activity_condition|
        role.has_permission?(:update, Activity)
      end

      can :destroy, PhaseActivityCondition do |_phase_activity_condition|
        role.has_permission?(:deactivate, Activity)
      end

      can :index, PhaseActivityCondition do |_phase_activity_condition|
        role.has_permission?(:show, Activity)
      end
    end
  end
