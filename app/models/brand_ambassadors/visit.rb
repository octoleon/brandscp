...
class BrandAmbassadors::Visit < ActiveRecord::Base
  ...
  searchable if: :active do
    ...
    integer :brand_ids, multiple: true do
      campaign.nil? ? -1 : campaign.brands.map(&:id)
    end
    integer :area_id do
  ...
  def self.do_search(params, _include_facets = false)
    solr_search(include: [:campaign, :area, company_user: :user]) do
      ...
      if (params.key?(:brand) && params[:brand].present?) || (params.key?(:brand_portfolio) && params[:brand_portfolio].present?)
        brand_ids = params[:brand] || []
        brand_ids += BrandPortfolio.where(id: params[:brand_portfolio]).joins(:brands).pluck('brands.id') if params.key?(:brand_portfolio) && params[:brand_portfolio].any?
        brand_ids = [brand_ids] unless brand_ids.is_a?(Array)

        with :brand_ids, brand_ids.uniq
      end
