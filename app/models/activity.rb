...
class Activity < ActiveRecord::Base
  ...
  def current_place
    place.merged_with_place_id.present? ? Place.find(place.merged_with_place_id) : place if place.present?
  end
  ...
  def searchable_params
    [:activity_type, :only_active]
  end
  class << self
    def do_search(params)
      # Setting whether to check only active events or not
      solr_search(include: [:campaign, :activity_type, company_user: :user]) do
        ...
        if params.key?(:venue) && params[:venue].present?
          venue_place_id = Venue.where(id: params[:venue]).pluck(:place_id)
          places_ids = venue_place_id + Place.where(merged_with_place_id: venue_place_id).pluck(:id)
          with :place_id, places_ids + [0]
        end
        ...
        any_of do
          all_of do
            ...
            with :events_active, true if params.key?(:only_active) && params[:only_active]
          end
