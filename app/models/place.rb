...
class Place < ActiveRecord::Base
  ...
  def name_with_location(sep = ', ')
    [name, [street_number, route, city, (country == 'US' ? state : state_name), country_name].compact.uniq.reject(&:empty?).join(', ')].join(sep)
  end
  ...
  def merge(place)
    ...
    self.class.connection.transaction do
      ...
      Venue.where(place_id: place.id).each do |venue|
        ...
        venues_to_destroy.push(venue.id)
      end
      Venue.destroy(venues_to_destroy)
