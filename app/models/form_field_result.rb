...
class FormFieldResult < ActiveRecord::Base
  ...
  scope :for_event_campaign, -> (campaign, state = nil) {
    query = joins('INNER JOIN events ON events.id=form_field_results.resultable_id AND form_field_results.resultable_type=\'Event\'')
    if state.nil?
      query.where(events: { campaign_id: campaign })
    else
      query.where(events: { campaign_id: campaign, aasm_state: state })
    end
  }
