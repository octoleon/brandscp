class Api::V1::FormFieldOptionsController < Api::V1::FilteredController
   inherit_resources
#  skip_authorization_check only: [:all,:add_activity_to_campaign,:add_activity_to_activity,:add_form_field_to_activity,:remove_sequence_from_activity_by_sequence,:modify_position_in_sequence_by_sequence_id,:add_condition_to_sequence]
  ...
  def show
	render json: FormFieldsOptions.find(params[:id])
  end
  ...
  def permitted_params
    params.permit(form_field_options: [:id,:form_field_id, :name,:ordering,:option_type])[:form_field_options]    
  end
