module Html
  class EventPresenter < BasePresenter
    ...
    def team_members_avatars (event_id)
      return if users.nil? && teams.nil?
      team_users = teams.map { |team| team.users}.flatten
      left = users.size + teams.size - 3
      avatars = (users + team_users).uniq.sort_by { |u| u.first_name}

      team_member_avatars = (users + teams).map.with_index do |team_member,index|
          if users.include?(team_member)
            h.content_tag(:div, id: "event-member-#{team_member.id}", class: 'user-avatar') do
              item =
                  h.content_tag(:div, class: 'user-type') do
                    team_member.initial_name
                  end
                  item += h.content_tag(:div, team_member.first_name + ' ' + team_member.last_name, style: "margin-left: 30px;") if index >= 3
                  if index < 3
                    item += h.content_tag(:div, class:'more-user-menu dropdown-menu', id: "collapse-more-user-#{event_id}") do
                      h.content_tag(:div, class: 'big-avatar') do
                        team_member.initial_name
                      end +
                      h.content_tag(:div, team_member.first_name + ' ' + team_member.last_name) +
                      h.content_tag(:span,  team_member.role.name ) + h.raw('<br/>') +
                      h.content_tag(:span,  team_member.simple_address)
                    end.html_safe
                  end
              item
            end
          else
            h.content_tag(:div, id: "event-team-#{team_member.id}", class: 'user-avatar') do
              item =
                  h.content_tag(:div, class: 'user-type') do
                    h.content_tag(:span, team_member.users.first.initial_name)
                  end
              item += h.content_tag(:div, 'Team ' +  team_member.name, style: "margin-left: 30px;") if index >= 3
              if index < 3
                item += h.content_tag(:div, class:'more-user-menu dropdown-menu', id: "collapse-more-user-#{event_id}") do
                    h.content_tag(:div, 'Team ' +  team_member.name) +
                    h.content_tag(:span, "#{team_member.users.size} members")
                end.html_safe
              end
              item
            end
      ...
      if left < 1
        team_member_avatars.take(3).join.html_safe
      else
        team_member_avatars.take(3).join.html_safe +  h.content_tag(:div, id:"more-users", class: 'dropdown user-avatar', style: 'top: -10px;') do
          h.link_to(" +#{left} more...", '#', class: 'dropdown-toggle', data: { toggle: 'dropdown', target: "collapse-more-user-#{event_id}" }) +
          h.content_tag(:ul, class:'more-user-menu more-menu dropdown-menu', id: "collapse-more-user-#{event_id}") do
            team_member_avatars.drop(3).map  do |item|
                h.content_tag(:li, item)
            end.join.html_safe
          end.html_safe
    ...
    def current_phase_activities
      puts '#' * 50 + ':CURRENT_PHASE_ACTIVITIES'
      puts phases[:phases][current_phase][:activities].inspect
      phases[:phases][current_phase][:activities]
    end
    ...
    def phase_steps(phase, index, steps)
      ...
      steps.map do |step|
        button = h.content_tag(:div,
                               class: phase_step_classes(step, steps.last[:id], steps.first[:id]),
                               data: { toggle: 'tooltip',
    ...
    def phase_step_classes(step, step_last_id, step_first_id)
      [
        'step',
    ...
    def phases_with_accessible_steps
      phases[:phases].map do |phase_name, phase_info|
        [phase_name, user_accessible_steps(phase_info[:activities]), phase_info[:phase_model]]
      end
    end
    ...
    def render_nav_phase(phase, i)
      ...
      h.link_to_if(i <= index_phase,
                   h.content_tag(:div, class: phase_classes(phase, i, index_phase)) do
                     (if phase[2].complete?(@model)
                        h.content_tag(:div, '', class: 'icon-check-circle')
                      else
                        phase_number = "#{i + 1}#{icon(:lock) if phase[2].locked?(@model)}".html_safe
                        h.content_tag(:span, phase_number.html_safe, class: 'id')
                      end) + phase[0][0,9].upcase
                   end, h.phase_event_path(@model, phase: phase[0], return: h.return_path)) + phase_steps(phase[0], i, phase[1])
    end

    def phase_classes(phase, i, index_phase)
      [
        'step', 'phase-id',
        ('active' if phase[0] == phases[:current_phase]),
        ('locked' if phase[2].locked?(@model))
      ].compact.join(' ')
    end

    def phase_buttons(phase)
      buttons =
        if phase[2].requires_approval
          if phase[2].submitted?
    ...
    def guided_message(phase, step)
      guided_message_presenter.send("#{phase}_#{step[:id]}") if guided_message_presenter.respond_to? "#{phase}_#{step[:id]}".to_sym
    end
